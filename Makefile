...
MODULES=account-alerts account-dashboards account-health-checks account-metrics env-alerts env-dashboards env-health-checks env-metrics singleton-alerts singleton-health-checks
ACCOUNTS=hca-id hca-prod humancellatlas

all: check

setup: ## set up working directory by installing dependencies
	curl -s https://raw.githubusercontent.com/chanzuckerberg/fogg/master/download.sh | bash -s -- -b .fogg/bin v0.36.9
	.fogg/bin/fogg setup
.PHONY: setup
